...
def render_sidebar() -> tuple[str, str, str]:
    """Render sidebar controls; return (symbol, period, interval)."""
    with st.sidebar:
...
def _extract_current_state(results: pd.DataFrame) -> tuple:
    """Extract the current regime and confidence from the last row of results."""
...
def render_metrics_row(results: pd.DataFrame, pipeline) -> None:
    """Render the four top-level metric cards."""
    current_regime, confidence = _extract_current_state(results)
...
def render_charts(df: pd.DataFrame, results: pd.DataFrame, pipeline) -> None:
    """Render price/regime, probability, transition matrix, and feature charts."""
...
def render_forecast_section(pipeline) -> None:
    """Render the regime forecast panel."""
    st.divider()
...
def _collect_signals(results: pd.DataFrame) -> list:
    """Run SignalDetector over all result rows and return detected signals."""
...
def render_signals_section(results: pd.DataFrame) -> None:
    """Render the signals & alerts panel."""
    st.divider()
    ...
    if signals:
        ...
        for signal in reversed(signals[-10:]):
            st.info(
                f"{icon_map.get(signal.signal_type, '•')} **{signal.signal_type.name}** — {signal.message}\n\n"
...
def render_data_inspector(results: pd.DataFrame) -> None:
    """Render the collapsible data inspector table."""
...
def main():
    ...
    if st.session_state.run_pipeline:
        ...
        st.divider()
        render_metrics_row(results, pipeline)
        st.divider()
        render_charts(df, results, pipeline)
        render_forecast_section(pipeline)
        render_signals_section(results)
