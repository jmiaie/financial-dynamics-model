...
**How it was made.** 2026-09-17, node Hai (Tailscale 100.111.189.7), the repo's own acquisition script
unmodified, yfinance 1.7.0, parameters identical to v1 (`--start 2015-01-01 --end 2026-01-01`,
`interval 1d`, `auto_adjust True`, `actions True`, `repair False`, `keepna True`).
...
v1 artifacts, or used to recompute the consumed primary result. Either:

1. **Recover the true v1 payload** from whatever machine produced the primary SPY result and verify it
   against `data/manifests/yf_fd_etfs_daily_2015_2025_v1.json` per-file sha256. If it matches, run the
   cross-asset robustness on it — exact comparability, no relabeling needed. **This is strictly
   preferred, and it is an owner question: does that machine still hold `data/raw/`?**
2. **Or adopt this as v2**, document the delta (this file), keep the SPY v1 result as the primary
   consumed historical result, and label every output computed from this payload as pre-specified
   robustness / post-primary characterization.
...
symbols — that is the point, and it is why the payload is committed here rather than left in `data/raw/`.
