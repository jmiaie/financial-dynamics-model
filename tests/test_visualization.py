...
class TestPhaseSpace3D:
    ...
    def test_animation_frames(self, sample_features, sample_regimes, sample_centroids):
        fig = build_phase_space_3d(sample_features, sample_regimes, sample_centroids, animate=True)
        assert len(fig.frames) > 0

    def test_with_basins_and_arrows(self, sample_features, sample_regimes, sample_centroids):
    ...
    def test_disable_basins_and_arrows(self, sample_features, sample_regimes, sample_centroids):
        T = np.eye(4)
        fig = build_phase_space_3d(
            sample_features,
            sample_regimes,
    ...
    def test_regime_transition_markers(self, sample_centroids):
        ...
        regimes = [Regime.CALM_TREND] * 20 + [Regime.RISK_OFF] * 20 + [Regime.CHOP] * 20
        fig = build_phase_space_3d(
            features,
            regimes,
            sample_centroids,
            show_transitions_markers=True,
            show_basins=False,
            show_stationary_halos=False,
            show_loadings=False,
        )
