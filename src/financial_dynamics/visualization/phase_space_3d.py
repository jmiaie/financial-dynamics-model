...
def build_phase_space_3d(
    ...
    projected, centroid_proj, pca = fit_pca_projection(feature_history, centroids, n_components=3)
    ...
    centroid_dists = np.array(
        [np.min(np.linalg.norm(centroid_proj - p, axis=1)) for p in projected]
    )
    ...
    for j, name in enumerate(FEATURE_NAMES):
        ...
        traces.append(
            go.Scatter3d(
                ...
                textfont=dict(color=color, size=10, family="Inter, system-ui, sans-serif"),
                showlegend=False,
                hovertemplate=(
...
def _build_transition_markers(
    ...
    labels = [f"{REGIME_NAMES[regimes[i - 1]]} → {REGIME_NAMES[regimes[i]]}" for i in changes]
    return go.Scatter3d(
        x=pts[:, 0],
        y=pts[:, 1],
        z=pts[:, 2],
...
def _build_velocity_trajectory(
    ...
    return go.Scatter3d(
        ...
        hovertemplate=(
            "Speed: %{customdata[0]:.3f}<br>Dist to attractor: %{customdata[1]:.2f}<extra></extra>"
        ),
        showlegend=True,
        legendgroup="trail",
    )
...
def _build_comet_trail(
    ...
    colors = [f"rgba(20, 184, 166, {o:.3f})" for o in opacities]
...
def _build_points(
    ...
    for regime in Regime:
        ...
        rgb = REGIME_COLORS_RGB[regime]
        marker_colors = [f"rgba({rgb[0]}, {rgb[1]}, {rgb[2]}, {o:.2f})" for o in opacities]
        traces.append(
            go.Scatter3d(
                x=pts[:, 0],
                y=pts[:, 1],
...
def _build_centroid_markers(
    ...
    pi = (
        _compute_stationary(transition_matrix)
    ...
    for k in range(step, n + 1, step):
        ...
        for regime in Regime:
            ...
            if mask.any():
                ...
                rgb = REGIME_COLORS_RGB[regime]
                opacities = 0.2 + 0.55 * np.clip(conf, 0.0, 1.0)
                colors = [f"rgba({rgb[0]}, {rgb[1]}, {rgb[2]}, {o:.2f})" for o in opacities]
                frame_data.append(
                    go.Scatter3d(
                        x=pts[:, 0],
                        y=pts[:, 1],
        ...
        frame_data.append(
            go.Scatter3d(
                ...
                line=dict(
                    color=decay,
                    colorscale=[[0, "rgba(71, 85, 105, 0.0)"], [1.0, "rgba(20, 184, 166, 0.9)"]],
                    width=3.5,
                ),
            )
        )
