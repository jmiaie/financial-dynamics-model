...
class FeatureEngine:
    ...
    def _update_buffers(self, ohlcv: dict[str, float]) -> None:
        """Append close and any reference-asset close values to internal buffers."""
        if "close" not in ohlcv:
            raise KeyError(
                f"Bar is missing required key 'close'. "
                f"Got keys: {sorted(ohlcv.keys())}"
            )
    ...
    def _build_ref_returns(self) -> dict[str, pd.Series] | None:
        """Build per-reference-asset return series from internal buffers.

        Returns None if no reference buffers have enough warmup data.
        """
    ...
    def _compute_raw_features(
        ...
        returns: pd.Series,
        ref_returns: dict[str, pd.Series] | None,
    ) -> np.ndarray:
        """Compute the 5-element raw feature array from buffered price series."""
    ...
    def update(self, bar_state: BarState) -> BarState:
        ...
        raw = self._compute_raw_features(close_series, returns, ref_returns)
